...
public:
  ...
  //ctors
  NavGraphNode():m_ExtraInfo(extra_info()),m_Walkable(true){}
  ...
  {
    char buffer[50];
    stream >> buffer >> m_iIndex >> buffer >> m_vPosition.x >> buffer >> m_vPosition.y>> buffer >> m_Walkable;
  ...
  friend std::ostream& operator<<(std::ostream& os, const NavGraphNode& n)
  {
    os << 
		"Index: " << n.m_iIndex << 
		" PosX: " << n.m_vPosition.x << 
		" PosY: " << n.m_vPosition.y << 
		" Walkable: "<< n.m_Walkable <<
		std::endl;
